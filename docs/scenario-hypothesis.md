...
**이렇게 이어질 때 / 다르게 갈 때** → 업종 파급 → **다시 볼 종목**까지 펼치는 **학습·시황 읽기** 기능.

**가설 검증이 아님.** 시그널·봇 미반영.  
문장은 **관리자 수동 생성 시 Sonnet 1회**(앞뒤 이어 읽는 쉬운 한국어).  
**관심%·갈래%·관심 종목(밸류체인)은 룰.** 일일 자동 LLM 호출 없음.
...
| 갈래 | 항상 `path`+`alt` 쌍. 갈래% + `emphasized`(지금 지표와 더 가까운 쪽) |
| 종목 | outcome 아래 `action` — 밸류체인 국내 대표 티커(추천 아님) |
| 상태 배지 | 없음. 트리·해설·%·종목만 |
