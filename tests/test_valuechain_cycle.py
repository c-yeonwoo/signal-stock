...
def test_cycle_position_returns_lead_sectors():
    ...
    # 지표 부족 시 lead_sectors 빈 리스트(키 존재 보장)
    assert cycle.position([])["lead_sectors"] == []


def test_valuechain_tags_favored_by_cycle(monkeypatch):
...
def test_valuechain_neutral_when_cycle_unknown(monkeypatch):
