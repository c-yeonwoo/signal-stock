"""두뇌 레이어 — 엔진 헬스 스냅샷(자가 진단의 score→diagnose 골격).

코어 파이프라인(소스→팩터→게이트→엔진→실측 트래커→진단→제안 루프)을 노드 그래프로 모델링하고,
각 노드의 헬스 상태와 규칙 기반 findings를 낸다. 이 모듈의 build()는 읽기 전용 헬스다.
지속 draft·승인 적용은 brain_proposals(+관리자 UI)가 담당한다 — 자동 적용 없음.
LLM 진단(opus)은 트래커가 성숙한 뒤 붙일 훅으로 남겨둔다.
